...
export const AdminApprovedPostsScreen = () => {
  ...
  const getPostStatus = (post) => {
    if (post.isActive) {
      return { label: 'Активно', color: '#50C878', bg: '#E8F5E9' };
    }
    if (post.tariff === null || !post.isPayed) {
      return { label: 'Не оплачено', color: '#FF9800', bg: '#FFF3E0' };
    }
    return { label: 'Неактивно', color: '#9E9E9E', bg: '#F5F5F5' };
  };

  const renderApprovedPostCard = (item, index) => {
        ...
        >
          <View style={styles.headerContent}>
            ...
            <View style={styles.headerText}>
              <Text style={styles.headerTitle}>Одобренные объявления</Text>
              <Text style={styles.headerSubtitle}>
                {approvedCount} из {statsData.approved_posts || 0} объявлений
              </Text>
            </View>
          </View>
        </LinearGradient>
      ...
      <View style={styles.infoCard}>
        <View style={styles.infoCardItem}>
          ...
          <Text style={styles.infoCardText}>
            Все одобренные администратором объявления
          </Text>
        </View>
      </View>
      ...
      <View style={styles.postsContainer}>
        {approvedPosts && approvedPosts.length === 0 ? (
          <View style={styles.emptyContainer}>
            ...
            <Text style={styles.emptyTitle}>Нет одобренных объявлений</Text>
            <Text style={styles.emptySubtitle}>
              Все объявления, одобренные администратором, будут отображаться здесь
            </Text>
          </View>
        ) : (
          approvedPosts && approvedPosts.map((item, index) => renderApprovedPostCard(item, index))
        )}
      </View>
    </ScrollView>
  );
